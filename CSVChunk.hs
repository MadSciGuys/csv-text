...
mapLinesSplitFile :: Int64 -> FilePath -> Handle -> (BC -> BC -> BC) -> IO ()
mapLinesSplitFile chunkSize inPath outHandle toBC = do
  (hdr, chunks) <- splitFileIO chunkSize inPath
  tmps <- flip MP.mapM chunks $ \ chunk -> do 
    let tmp = (chunk++".tmp")
    BC.writeFile tmp . BC.intercalate "\n" 
      . map (toBC hdr) . BC.lines =<< BC.readFile chunk
    removeFile chunk
    return tmp
  putOut hdr
  flip mapM_ tmps $ \ tmp -> do
    putOut =<< BC.readFile tmp
    removeFile tmp
  where putOut = BC.hPutStrLn outHandle

foldMapSplitFile :: (Monoid m) => Int64 -> FilePath -> (BC -> BC -> m) -> IO m
foldMapSplitFile chunkSize inPath toMonoid = withSplit chunkSize inPath
  $ uncurry $ \ hdr -> fmap mconcat 
      . MP.mapM (fmap (foldMap (toMonoid hdr) . BC.lines) . BC.readFile)
